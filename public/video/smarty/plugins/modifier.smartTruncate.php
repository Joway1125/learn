<?php
/* 
 * smarty中用truncate来截取含有中英文的字符串，可能会出现中文乱码问题、字符串截取长度不一问题，
 * 下面是新建个扩展函数，或修改原Truncate函数方法也可以的。
 * 扩展smarty/plugins目录下自己新建一个文件，写个函数，
 * 修改后的smartTruncate： 文件名：modifier.smartTruncate.php
 */
function smarty_modifier_smartTruncate($string, $length = 80, $etc = '...',$break_words = false, $middle = false) {     if ($length == 0)         return '';     if (smartStrlen($string) > $length) {
...
function smartStrlen($string)
{
    ...
    for($i = 0; $i < strlen($string); $i += $bytes)     {         $bytes = ord(substr($string, $i, 1)) > 127 ? $number : 1;
...
function smartSubstr($string, $start, $length = null)
{
    ...
    {
        if($start <= 0)         {             break;         }         $bytes = ord(substr($string, $i, 1)) > 127 ? $number : 1;
    ...
    else
    {
        ...
        {
            if($length <= 0)             {                 break;             }             if(($bytes = ord(substr($string, $j, 1)) > 127 ? $number : 1) > 1)
            {
                if($length < 1.0)
                {
                    break;
